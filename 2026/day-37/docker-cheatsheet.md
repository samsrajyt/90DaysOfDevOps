...
| Multiple env vars | `docker run -e DB_HOST=localhost -e DB_PORT=5432 mysql` |
...
| Create bridge network | `docker network create --driver bridge <network name>` |
| Inspect network | `docker network inspect <netwrok name>` |
...
| Removing containers & networks along with volume mounts | `docker compose down -v` |
